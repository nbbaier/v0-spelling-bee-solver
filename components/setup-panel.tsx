...
export function SetupPanel({ onLoad }: { onLoad: (p: Puzzle) => void }) {
      ...
      <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
        <div className="mb-6 space-y-1.5">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Paste the two-letter grid and the hint list copied from sbsolver. Your progress is saved in this browser.
          </p>
        </div>
        ...
        <div className="space-y-5">
          ...
          <Button onClick={handleLoad} className="w-full" disabled={!matrixText.trim() || !hintsText.trim()}>
            Load puzzle
          </Button>
        </div>
      </div>
