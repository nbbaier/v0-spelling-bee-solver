...
export type FetchPuzzleResult =
  | { ok: true; matrixText: string; hintsText: string; date: string | null; failedPrefixes: string[] }
  | { ok: false; error: string }

export async function fetchPuzzleFromUrlAction(url: string): Promise<FetchPuzzleResult> {
  try {
    const { matrixText, hintsText, date, failedPrefixes } = await scrapePuzzle(url)
    return { ok: true, matrixText, hintsText, date, failedPrefixes }
  } catch (e) {
    return { ok: false, error: e instanceof Error ? e.message : "Could not fetch that puzzle." }
  }
}
